"""Shared library vs per-workspace credentials for AI-PRO-SKILLS.

**Library (shared):** ``~/.ai-pro-skills/<skill>/`` — CLI, action modules, deps.
...
Usage from a skill folder::
    ...
    _ROOT = _LIB.parent  # AI-PRO-SKILLS root (sibling of common/)
    if str(_ROOT) not in sys.path:
        sys.path.insert(0, str(_ROOT))

    from common.skill_home import SkillHome

    home = SkillHome("reddit", library_home=_LIB)
    env = home.env_path()
"""
...
class SkillHome:
    ...
    def env_override_var(self) -> str:
        """Back-compat: override var for the ``.env`` file (e.g. reddit → REDDIT_ENV_PATH)."""
