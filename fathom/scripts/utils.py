...
def _load_env() -> None:
    """Load ``.env`` via the shared ``common/skill_home.py`` when available.

    Resolution is location-aware (registered skill dir / workspace / Hermes
    profile). Falls back to a local ``.env`` next to the scripts or skill root,
    then to python-dotenv's default search, so the script also works standalone.
    """
    try:
        from _skill_home import env_path as _resolve  # shared code

        candidate = Path(_resolve())
