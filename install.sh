#!/usr/bin/env bash
#
# AI-PRO-SKILLS — minimal shared-env helper. THREE commands only:
#
...
#
#   ~/.ai-pro-skills/            shared library root ($AI_SKILLS_HOME)
#   ~/.ai-pro-skills/ext/<name>  external skill repos, cloned ONCE (fetch)
#   ~/.ai-pro-skills/.venv       shared Python venv (pip init)
#
...
AI_SKILLS_HOME="${AI_SKILLS_HOME:-$HOME/.ai-pro-skills}"
EXT_DIR="$AI_SKILLS_HOME/ext"
VENV_DIR="$AI_SKILLS_HOME/.venv"

log() { printf '\033[1;36m[ai-pro-skills]\033[0m %s\n' "$*"; }
err() { printf '\033[1;31m[ai-pro-skills] error:\033[0m %s\n' "$*" >&2; }
